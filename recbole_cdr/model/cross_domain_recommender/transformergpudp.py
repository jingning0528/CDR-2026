...
class TransformerGPUDP(CrossDomainRecommender):
    ...
    def __init__(self, config, dataset,
                 num_heads=8, num_layers=1, dim_feedforward=2048,
                 transformer_dropout=0.1, transformer_activation='relu'):
        super(TransformerGPUDP, self).__init__(config, dataset)
        ...
        self.num_heads = 8  # You can choose any appropriate number
        assert self.embedding_size % self.num_heads == 0, "Embedding size should be divisible by number of heads"
        self.transformer_block = TransformerBlock(self.embedding_size, self.num_heads)

        # Register after the final Transformer is constructed so DP applies to
        # every trainable parameter, including the replacement block above.
        for param in self.parameters():
            param.register_hook(self.add_dp_noise_)

    def add_dp_noise_(self, grad):
        """
        This method applies gradient clipping followed by noise addition for differential privacy.
        """
        grad_norm = torch.norm(grad, p=2)
        grad = grad * (self.clip_norm / (grad_norm + 1e-6))

        noise = torch.normal(0, self.sigma, size=grad.shape, device=grad.device)
        return grad + noise
    ...
    def neumf_forward(self, user, item, domain='source'):
        ...
        user_e = self.attention_layer(user_e, item_e)
        item_e = self.attention_layer(item_e, user_e)

        user_e = self.transformer_block(user_e.unsqueeze(1)).squeeze(1)
        item_e = self.transformer_block(item_e.unsqueeze(1)).squeeze(1)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.base_model == 'NeuMF':
            ...
            regularization = (self.source_user_embedding.weight.norm() +
                              self.source_item_embedding.weight.norm() +
                              self.target_user_embedding.weight.norm() +
                              self.target_item_embedding.weight.norm())

            return loss_s * self.alpha + loss_t * (1 - self.alpha) + self.regularization_weight * regularization
            # return loss_s * self.alpha + loss_t * (1 - self.alpha)
