...
def _cat_interactions_recbole_1_0(interactions):
    """Concatenate interactions without RecBole 1.0's key-type ambiguity.

    RecBole 1.0.1's ``cat_interactions`` accesses every column through
    ``Interaction.__getitem__``. If a cross-domain column key is not a native
    Python string, RecBole mistakes that key for a row index and can return an
    ``Interaction`` instead of a tensor. Access the underlying mapping directly
    so sampled evaluation always receives tensors.
    """
...
class CrossDomainNegSampleEvalDataLoader(NegSampleEvalDataLoader):
    ...
    def _next_batch_data(self):
        ...
        # This is the four-part batch expected by RecBole 1.0.1 Trainer.evaluate.
